...
func main() {
	for {
		...
		case 2:
			fmt.Println("进入客户端模式...")
			fmt.Println("请选择客户端传输的是单个小型文件还是文件夹：")
			fmt.Println("选项1：传输单个小型文件")
			fmt.Println("选项2：传输文件夹")
			clientChoice, err := clientChoice()
			if err != nil {
				fmt.Println("输入无效，请输入有效的数字选项 (1-3)：")
				continue
			}
			switch clientChoice {
			case 1:
				fmt.Println("进入传输单个小型文件模式中...")
				role_client.Client()
			case 2:
				fmt.Println("进入传输文件夹模式中...")
				role_client.Client()
			}
		case 3:
			fmt.Println("正在退出MDA...")
...
func clientChoice() (int, error) {
	...
	// 转换为整数
	clientChoice, err := strconv.Atoi(choiceStr)
	if err != nil {
		return 0, fmt.Errorf("无法将输入解析为数字：%v", err)
	}

	return clientChoice, nil
}
